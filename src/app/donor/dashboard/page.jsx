...
export default function DonorDashboard() {
  const { totalPatients, fetchStats, loading, totalApproved } = useDonorAuthStore();
    ...
    <section className="space-y-6 text-white">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-black uppercase tracking-widest text-white italic">Network Analytics</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-surface-2 border border-white/5 rounded-xl p-6 shadow-xl transition-all duration-300 hover:border-red-500/30 group relative overflow-hidden flex flex-col justify-between h-40">
             <div className="flex justify-between items-start mb-6">
               <h3 className="text-[9px] font-black text-text-dim uppercase tracking-[0.2em]">Total Patients in Need</h3>
             </div>
